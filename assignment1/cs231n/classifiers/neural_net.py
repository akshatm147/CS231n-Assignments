...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # Calculate fix for softmax scores
    ...
    correct_class_score = np.choose(y, shift_scores.T) # Size N vector
    loss = -correct_class_score + np.log(np.sum(np.exp(shift_scores), axis=1))
    loss = np.sum(loss)
    
    # Average the loss and add the regularization term
    loss /= N
    ...
    # Calculate dScore : The grdient wrt the softmax scores
    dScore = softmax_scores
    dScore[range(N), y] -= 1
    dScore /= N # Average over batch
    
    # Backprop dScore to calculate dW2 and add the regularization term
    dW2 = np.dot(relu_1_activation.T, dScore)
    dW2 += 2 * reg * W2
    grads['W2'] = dW2
    
    # Backprop scores to calculate db2
    db2 = dScore * 1
    ...
    # Backprop dReLU1 to calculate dW1 and regularization derivative
  ...
  def train(self, X, y, X_val, y_val,
    ...
    # List of integers between 0 and len(X)
    ...
    for it in range(num_iters):
      ...
      # Choose batch size random values from X_indices
